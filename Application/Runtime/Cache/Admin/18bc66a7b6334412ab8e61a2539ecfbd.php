		...
		<div class="mt-20">
			<table class="table table-border table-bordered table-bg table-hover table-sort">
				...
				<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="text-c va-m">
						...
						<td class="td-status">
							<?php if($vo['or_state'] == 1): ?><span class="label label-success radius">已点击送餐</span>
								<?php elseif($vo['or_state'] == 0): ?>
									<span class="label label-defaunt radius">快点送餐吧！</span>
								<?php else: ?>
									<span class="label label-success radius">已签收</span><?php endif; ?>
						</td>
						<td class="td-manage">
							<?php if($vo['or_state'] == 1): ?><a style="text-decoration:none" onClick="order_stop(this,<?php echo ($vo["id"]); ?>)" href="javascript:;" title="取消送餐"><i class="Hui-iconfont">&#xe6de;</i></a>
								<?php elseif($vo['or_state'] == 0): ?>
									<a style="text-decoration:none" onClick="order_start(this,<?php echo ($vo["id"]); ?>)" href="javascript:;" title="点击送餐"><i class="Hui-iconfont">&#xe603;</i></a>
								<?php else: ?>
									<span class="label label-success radius">已签收</span><?php endif; ?>
