	...
	<form enctype="multipart/form-data"  action="pictured_add_cp_in" method="post" class="form form-horizontal" id="form-article-add">
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" value="<?php echo ($product["name"]); ?>" placeholder="" id="name" name="name">
			</div>
		</div>
		...
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" name="price" id="price" placeholder="" value="<?php echo ($product["price"]); ?>" class="input-text" style="width:90%">
				元</div>
		</div>
		...
		<div class="row cl">
			<div class="col-xs-8 col-sm-9 col-xs-offset-4 col-sm-offset-2">
				...
				<input onClick="upda_pro();"  class="btn btn-primary radius" id="<?php echo ($product["id"]); ?>" type="submit" value="修改"></input>
...
<script type="text/javascript">
	function upda_pro(){
		$.post("<?php echo U('Product/product_edit_cp_in');?>",{'id': $('#id').val(),'name':$('#name').val(),'price':$('#price').val(),'type_id':$("#type_id option:selected").val()},function(r){
            if(r.code == 1){
                layer.msg("修改成功！");layer_close();
            }else {
                layer.msg("修改失败！");layer_close();
            }
		});
	}
